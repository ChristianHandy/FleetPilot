...
try:
    disktool_core.subprocess.run = lambda *args, **kwargs: SimpleNamespace(
        returncode=0,
        stdout="VALIDATION_SUMMARY samples=8 bytes=32768 bad=0 size=1048576\n",
    )
    blocks, bad_blocks, summary = disktool_core.validate_blocks("sdb")
    assert len(blocks) == 8
    assert bad_blocks == []
