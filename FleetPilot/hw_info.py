...
def register_routes(app, login_required, csrf=None):
    ...
    def hw_overview():
        ...
        # Full list of configured servers (registry + hosts.json), not just
        # ones that happen to already have cached hardware data — otherwise
        # a server added anywhere else (Manage Hosts, VM Controller, ...)
        # never appears here at all, since nothing ever triggers a first scan.
        configured = {}
        try:
            import server_registry as _reg_mod
            reg = _reg_mod.get_registry(data_dir)
            for s in reg.list_servers():
                configured[s['name']] = s.get('host', '')
        except Exception as e:
            logger.warning(f"[HwInfo] Could not read server registry: {e}")
        try:
            with open(os.path.join(data_dir, 'hosts.json')) as f:
                hosts = json.load(f)
            for name, h in hosts.items():
                configured.setdefault(name, h.get('host', ''))
        except Exception:
            pass

        cached_by_name = {c['server_name']: c for c in _collector.list_cached()}
        merged = []
        for name, host in configured.items():
            c = cached_by_name.get(name)
            merged.append({
                'server_name': name,
                'host': (c['host'] if c else host) or host,
                'collected_at': c['collected_at'] if c else None,
            })
        # Include any cached entries for servers no longer in the configured
        # list (e.g. deleted since their last scan) so their data isn't lost
        for name, c in cached_by_name.items():
            if name not in configured:
                merged.append({'server_name': name, 'host': c['host'], 'collected_at': c['collected_at']})
        merged.sort(key=lambda s: s['server_name'].lower())

        return render_template('hw_overview/index.html', servers=merged)
