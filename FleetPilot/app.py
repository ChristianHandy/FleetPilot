...
# Checks the repo root first (matches the documented `cp .env.example .env`
# setup step), then falls back to /opt/fleetpilot/.env for deployments that
# already rely on that path — previously only the /opt path was checked, so
# a .env created via the documented steps was silently ignored unless the
# app happened to be installed at exactly /opt/fleetpilot.
try:
...
def index():
    ...
    try:
        ...
        smart_summary = {}
    try:
        smart_disks = smart_manager.get_all_disks()
        for _d in smart_disks:
            size_gb = _d.get("size_gb")
            if size_gb:
                _d["capacity_display"] = f"{size_gb/1000:.1f} TB" if size_gb >= 1000 else f"{size_gb:.0f} GB"
            else:
                _d["capacity_display"] = "\u2014"
            _d["temp_display"] = f"{_d['temp']}\u00b0C" if _d.get("temp") else "\u2014"
    except Exception:
        smart_disks = []

    # Online/offline fleet counts (reuses the same is_online() check the
    # Update Dashboard uses — a short-timeout SSH probe per host)
    online_count = 0
    for h in hosts.values():
        try:
            if is_online(h.get("host", ""), h.get("user", "")):
                online_count += 1
        except Exception:
            pass
    offline_count = max(len(hosts) - online_count, 0)

    # "Updates Available" — hosts FleetPilot has never updated, or hasn't
    # updated in 30+ days. There's no per-host pending-package count in
    # this app (updates are applied on demand from the Update Dashboard,
    # not dry-run-checked), so this is the honest signal already on hand.
    ...
    for h in hosts.values():
        ...
        try:
            ...
            if datetime.now(last_dt.tzinfo) - last_dt > timedelta(days=30):
                updates_needed_count += 1
        except Exception:
            updates_needed_count += 1

    # "Warnings" — disks SMART has flagged as WARNING (CRITICAL/FAILED are
    # already surfaced via the Offline/status pills, so this stays focused).
    warnings_count = int((smart_summary.get("counts") or {}).get("WARNING", 0) or 0)

    # Local CPU/GPU/NVMe temps + fan RPMs for the Fan & Cooling widget
    # (reads sensors on the machine FleetPilot itself runs on)
    local_sensors = home_widgets.get_local_sensors()

    # Record today's snapshot and compute trend/sparkline data for the
    # quick-stat cards. Falls back to a flat "no change" trend until a
    # few days of real history have accumulated.
    stat_trends = home_widgets.get_trends(DATA_DIR, {
        "total": len(hosts),
        "online": online_count,
        "offline": offline_count,
        "updates": updates_needed_count,
    ...
    return render_template(
        ...
        updates_needed_count=updates_needed_count,
...
def _run_fp_update_bg(channel, do_restart):
    ...
    # NOTE: `git add -A` respects .gitignore, so DATA_DIR (data/) is never
    # swept into this — it's only ever plain untracked stray files.
    stashed = False
    try:
        ...
        if 'would be overwritten' in proc_dry.stderr or 'untracked working tree' in proc_dry.stderr:
            ...
            stash_proc = subprocess.run(['git', 'stash'], capture_output=True, text=True, cwd=app_dir)
            # "No local changes to save" means nothing was actually stashed —
            # only try to restore later if something really got pushed onto the stash.
            stashed = stash_proc.returncode == 0 and 'No local changes to save' not in stash_proc.stdout
            _fp_log('Stashed local untracked files.' if stashed else 'Nothing to stash.', 'info')
    except Exception as e:
    ...
    # Always try to restore whatever was stashed above, regardless of
    # whether the pull itself succeeded — previously this was never called
    # at all, so a stash created in step 2b was silently abandoned for good.
    if stashed:
        pop_proc = subprocess.run(['git', 'stash', 'pop'], capture_output=True, text=True, cwd=app_dir)
        if pop_proc.returncode == 0:
            _fp_log('Restored stashed local files.', 'info')
        else:
            # Don't lose the stash silently — leave it in the stash list and
            # tell the admin exactly how to recover it by hand.
            _fp_log('Could not automatically restore stashed files — they are '
                    'still safe in the git stash. Run `git stash list` / '
                    '`git stash pop` in the app directory to recover them.', 'error')
            _fp_log(pop_proc.stderr.strip(), 'error')
...
@app.route('/api/live')
def api_live():
    """Health check endpoint for HAProxy and monitoring."""
    return 'OK', 200
...
@app.route('/api/known_servers')
@login_required
def api_known_servers():
    """API: List servers from the central registry + hosts.json, for
    "quick-select known server" pickers (Hardware Monitor's Add Server
    modal, and similar). Wraps the existing _get_known_servers_for_module()
    helper, which was previously only used for server-side template
    context, not exposed to any client-side fetch() call."""
    module = request.args.get('module')
    try:
        servers = _get_known_servers_for_module(module)
        safe = [{
            'name': s.get('name', ''),
            'host': s.get('host', ''),
            'ip': s.get('host', ''),  # some pickers expect 'ip'
            'port': s.get('port', 22),
